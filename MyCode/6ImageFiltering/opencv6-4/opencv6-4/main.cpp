...
int main()
{
    ...
    cv::imshow("HorizontalSobelFilter(3×3)", sobelX);
    cv::waitKey();
    ...
    cv::Sobel(gray,        // 输入图像
              ...
              3,                // 正方形内核尺寸，必须为奇数
              0.4, 128);        // 缩放比例和偏移量(默认值为1和0)
    cv::imshow("VerticalSobelFilter(3×3)", sobelY);
    cv::waitKey();
    ...
    cv::imshow("SobelFilterL1Model", sobelL1Image);
    cv::waitKey();
    ...
    cv::imshow("SobelFilterL2Model", sobelL2Image);
    cv::waitKey();
    ...
    cv::imshow("Thresholding", sobelThresholded);
    cv::waitKey();
    ...
    cv::imshow("HorizontalSobelFilter(7×7)", sobelX);
